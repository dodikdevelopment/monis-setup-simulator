...
import { DESKS } from '../../data/workspaceData';
import { Check, Sparkles } from 'lucide-react';
import { motion } from 'framer-motion';

export default function DeskSelector() {
  const { selectedDeskId, selectedDeskColor, setDesk, setDeskColor } = useWorkspaceStore();
    ...
    <div className="flex flex-col gap-4 py-2">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-xs text-zinc-500 dark:text-zinc-400">
          The foundation of your workflow. Choose a shape and size that fits your body.
        </p>
      </div>
              ...
              {isSelected && (
                <div className="mt-4 pt-3 border-t border-zinc-200/50 dark:border-zinc-800/50 flex flex-col gap-2 animate-fadeIn">
                  ...
                  <div className="flex items-center gap-2">
                    {desk.colorOptions.map((opt) => {
                      const isActiveColor = selectedDeskColor === opt.name;
                      return (
                        <button
                          key={opt.name}
                          onClick={() => setDeskColor(opt.name)}
                        ...
                        >
                          <span
                            className={`h-3 w-3 rounded-full border border-white/20 ${opt.textureClass}`}
                            style={{
                              backgroundColor: opt.textureClass ? undefined : opt.value,
                            }}
                          />
                          {opt.name}
                        </button>
